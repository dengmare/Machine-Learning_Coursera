...
J = sum(sum(...
    ...
    )); %cost function without regulation

J = J + lambda / (2*m) * ( sum(sum(Theta1(:,2:end) .^2)) + sum(sum(Theta2(:, 2:end) .^2)) ); %cost function with regulation
...
for t = 1:m
    a1_t = X(t,:)';  %401x1
    z2_t = [1; Theta1 * a1_t];  %26x1
    
    a2_t = hiddenLayer(t,:)'; %26x1
    
    yMat_t = yMat(t,:)'; % vector of Tth output layer 10x1
    
    d3_t = outputLayer(t,:)' - yMat_t; %10x1
    
    d2_t = Theta2' * d3_t .* sigmoidGradient(z2_t);  %26x1
